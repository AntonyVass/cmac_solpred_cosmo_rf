...
# ----- set k folds
k <- nlevels(as.factor(masterSet$solute_name))
levs <- levels(as.factor(masterSet$solute_name))
...
for(i in 1:k){
  ...
  # Split train/test
  solute_to_remove <- levs[i]
  testSet <- masterSet[masterSet$solute_name == solute_to_remove,]
  trainingSet <- masterSet[!masterSet$solute_name == solute_to_remove,]
